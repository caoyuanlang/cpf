 ...
 * @LastEditTime: 2023-06-07 15:57:58
...
<template>
  <div class="playerContainer">
    ...
    <div class="People-counting">
      ...
      <div class="imgList">
        <img
          v-for="(url, index) in imgsArr.slice(0, 3)"
          :key="index"
          :src="url"
          alt=""
        />
        <div class="imgBtn">
          <!-- <div>下载</div> -->
          <div @click="imgClear">清空</div>
          <div @click="imgContrast">对比</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.$store.commit({
      type: "addattribute",
      content: { key: "channelId", value: this.channelCode },
    });
  },
  ...
  methods: {
    ...
    increaseImg(url) {
      let bool = this.imgsArr.some((item) => {
        return item === url;
      });
      ...
      this.imgsArr.length < 4
        ? (this.imgsArr = [...this.imgsArr, url])
        : this.$message.warning("只能添加四张照片！");
    },
    imgClear() {
      this.imgsArr = [];
    },
    imgContrast() {
      if (this.imgsArr.length >= 2) {
        this.$emit("update:dialogVisible", true);
        this.dialogVisible = true;
      } else {
        this.$message.warning("对比需要至少两张照片");
      }
    },
  },
};
