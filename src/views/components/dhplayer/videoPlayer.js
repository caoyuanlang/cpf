                ...
                case 'web.seekRecord':
                    hwndInfo.channelList.forEach((item, index) => {
                        if (item.snum === data.info.snum) {
                            ...
                            item.records && item.records.forEach((r, currentIndex) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.setting.request[type](getAjaxParam(param, type)).then(res => {
                if (!param.records || param.records === []) {
                    ...
                    this.setting.channelList[this.setting.channelList.findIndex(item => item.channelId === param.channelId)] = param
                }
                resolve(res)
            }).catch(err => {
                reject(err)
    ...
    VideoPlayer.fn = VideoPlayer.prototype = {
        ...
        startTalk: async function (snum = 0, type = 'talk') {
            ...
            this.setting.request[type]({ data: param }).then(res => {
                ...
                this.setting.request.stopTalk && this.setting.request.stopTalk(getAjaxParam(talkParam, 'stopTalk')).then(() => {
                    this.setting.request[type]({ data: param }).then(res => {
                        talkParam.session = res.session
                        this.talkByUrl({
                            redirect: false,
        ...
        cover: function () {
            ...
            if (window.dhPlayerControl.isPIframe) {
                ...
                var pIframe = null
                for (var i = 0; i < iframes.length; i++) {
                    let dom = iframes[i].contentWindow.document.getElementById(this.setting.videoId)
                    if (dom) {
                        pIframe = iframes[i]
                        break
                    }
